...
This package, currently on GitHub, adds engines for the `parsnip` pacakge so that we can fit variaus types of event time models. 
...
cart_fit <-
  ...
  set_engine("ipred", times = 50) %>%
  fit(Surv(age, adopted) ~ ., data = dogs)

predict(cart_fit, our_dogs, type = "survival", time = times) %>% 
  bind_cols(our_dogs %>% select(pooch)) %>%
  unnest(.pred_survival) %>%
  mutate(years = .time / 365) %>%
  ggplot(aes(x = years, y = .pred_survival, group = pooch, 
             col = pooch)) +
  geom_step(lwd = 1.25) + 
...
* Linear predictors are always increasing with time. tidymodels assumes that you can compute performce without knowing about the model type or other data. 

* Many survival ROC methods are not designed for assessing predictions. We will implement the one that is most approopriate. 

* Current specifications for baseline survival groups are very limited. We'll give an more versitile tool for declaring the baseline. 
